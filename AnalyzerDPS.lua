...
local function ShouldAutoStartFight(subevent, spellId, isPlayerSource, isPlayerDest, isAuraEvent, destFlags)
  -- Zapobiegaj startowaniu walki zaraz po zakończeniu poprzedniej (DoT-y, pet DMG)
  if Analyzer.lastFightEndTime then
    local timeSinceLastFight = GetTime() - Analyzer.lastFightEndTime
    if timeSinceLastFight < 3 then
      -- W ciągu 3s po zakończeniu walki, startuj tylko od castów gracza, nie od damage
  ...
  -- Priorytet 1: Damage events (najbardziej pewne)
  ...
  -- Priorytet 2: Hostile casts (atakujące spelle)
  if isPlayerSource then
  ...
  -- NIE startuj walki tylko od debuffa (problem z dotami na dummy)
  -- Debuff może być nałożony poza walką
  
  -- Priorytet 3: Jesteśmy w combat i castujemy
  ...
  -- Priorytet 4: Buffy w combat (nie debuffs!)
  if isAuraEvent and isPlayerDest and IsTrackedBuffSpell(spellId) then
    return true
  end
...
function Analyzer:SwitchReportTab(tabIndex)
  ...
  if frame.reportSuggestionsContent then
    frame.reportSuggestionsContent:SetShown(tabIndex == 2)
  end
  if frame.reportLogContent then
    frame.reportLogContent:SetShown(tabIndex == 3)
  end
  if frame.reportHistoryContent then
    frame.reportHistoryContent:SetShown(tabIndex == 4)
  end
end
...
function Analyzer:StartFight(startTimestamp)
  ...
  -- Reset score cache przy starcie walki i ustaw na początkową wartość
  self.cachedLiveScore = 100
  self.lastScoreUpdate = GetTime()
  
  -- Reset aktywnych wskazówek
  self.activeAdvices = {}
end
...
function Analyzer:EndFight(autoStopped)
  if not self.fight then
    return
  end
  ...
  -- Sprawdź czy walka była "prawdziwa" (minimum damage)
  ...
  -- Nie otwieraj okna przy auto-stop LUB jeśli walka była za krótka/bez damage
  if fightDuration >= 5 and reportBuilt and not autoStopped and wasRealFight then
  ...
  -- Reset cache score po zakończeniu walki
  self.cachedLiveScore = nil
...
function Analyzer:TrackDamage(amount)
  ...
  self.fight.lastDamageTime = GetTime()
  
  -- Ustaw flagę że walka naprawdę się rozpoczęła (mamy damage)
  if not self.fight.hasRealDamage and self.fight.damage >= 100 then
    self.fight.hasRealDamage = true
  end
end

function Analyzer:TrackSpellCast(spellId, timestamp)
  if self.fight then
    self.fight.lastCastTime = GetTime()
  end
  local module = self.activeModule
...
function Analyzer:RenderSuggestions(report)
  local ui = self.ui
  if not ui or not ui.suggestionsContainer then
    return
  end
  
  -- Wyczyść poprzednie sugestie
  if ui.suggestionItems then
    for _, item in ipairs(ui.suggestionItems) do
      item:Hide()
      item:SetParent(nil)
    end
  end
  ...
  local container = ui.suggestionsContainer
  local yOffset = 0
  
  if #report.issues == 0 then
    local noIssues = container:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    noIssues:SetPoint("TOPLEFT", 0, 0)
    noIssues:SetText("|cFF00FF00✓|r " .. self:L("NO_CRITICAL_ISSUES"))
    table.insert(ui.suggestionItems, noIssues)
  ...
  for i, issue in ipairs(report.issues) do
    ...
    item:SetSize(container:GetWidth(), 40)
    item:SetPoint("TOPLEFT", 0, yOffset)
    
    -- Ikona ostrzeżenia
    local icon = item:CreateTexture(nil, "ARTWORK")
    icon:SetSize(24, 24)
    icon:SetPoint("TOPLEFT", 4, -8)
    icon:SetTexture("Interface\\DialogFrame\\UI-Dialog-Icon-AlertNew")
    
    -- Numer
    local number = item:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    number:SetPoint("LEFT", icon, "RIGHT", 6, 0)
    number:SetText(tostring(i) .. ".")
    number:SetTextColor(1.00, 0.82, 0.00)
    
    -- Tekst sugestii
    local text = item:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    text:SetPoint("LEFT", number, "RIGHT", 6, 0)
    text:SetPoint("RIGHT", item, "RIGHT", -8, 0)
    ...
    -- Separator
    local sep = item:CreateTexture(nil, "BACKGROUND")
    sep:SetHeight(1)
    ...
    -- Dostosuj wysokość do tekstu
    local textHeight = text:GetStringHeight()
    item:SetHeight(math.max(40, textHeight + 20))
    
    table.insert(ui.suggestionItems, item)
...
function Analyzer:RenderReport(report)
  ...
  ui.score:SetText(string.format("%s: %d / 100", self:L("SCORE"), report.score))
  ui.score:SetTextColor(scoreColor[1], scoreColor[2], scoreColor[3])

  if report.dps and report.dps > 0 then
    ui.dps:SetText(string.format("%s (%s): %.0f", self:L("DPS"), report.dpsSource or "log", report.dps))
  else
    ui.dps:SetText(self:L("DPS") .. ": n/a")
  end
  ui.duration:SetText(string.format("%s: %.1fs", self:L("FIGHT_DURATION"), report.duration or 0))
  ...
  -- Renderuj issues obok metryk
  if ui.issuesText then
    local issuesLines = {}
    for _, issue in ipairs(report.issues or {}) do
      table.insert(issuesLines, "• " .. issue)
    end
    if #issuesLines == 0 then
      ui.issuesText:SetText("|cFF00FF00" .. self:L("NO_CRITICAL_ISSUES") .. "|r")
    else
      ui.issuesText:SetText(table.concat(issuesLines, "\n\n"))
    end
  end

  -- Renderuj sugestie w zakładce Sugestie
  self:RenderSuggestions(report)
  ...
  if #logLines == 0 then
    table.insert(logLines, self:L("NO_LOG"))
  end
...
function Analyzer:ClearCurrentReport()
  ...
  if self.ui then
    ...
    if self.ui.dps then
      self.ui.dps:SetText(self:L("DPS") .. ": --")
    end
...
function Analyzer:ApplyUISkin(frame, options)
  ...
  elseif skin.border then
    if skin.border.top then skin.border.top:Hide() end
    if skin.border.bottom then skin.border.bottom:Hide() end
    if skin.border.left then skin.border.left:Hide() end
    if skin.border.right then skin.border.right:Hide() end
  end
...
local function CreateMetricRow(parent, index)
  ...
  row.bar:SetMinMaxValues(0, 1)
  row.bar:SetValue(0)
  row.bar:SetSize(120, 10)
  row.bar:SetPoint("LEFT", 160, 0)
...
local function CreateReportFrame()
  ...
  frame.reportTabs[2] = CreateReportTab(reportContent, 2, Analyzer:L("REPORT_SUGGESTIONS"), function()
    Analyzer:SwitchReportTab(2)
  end)
  frame.reportTabs[3] = CreateReportTab(reportContent, 3, Analyzer:L("REPORT_LOG"), function()
    Analyzer:SwitchReportTab(3)
  end)
  frame.reportTabs[4] = CreateReportTab(reportContent, 4, Analyzer:L("REPORT_HISTORY"), function()
    Analyzer:SwitchReportTab(4)
  end)
  ...
  local reportSuggestionsContent = CreateFrame("Frame", nil, reportContent)
  reportSuggestionsContent:SetAllPoints()
  reportSuggestionsContent:Hide()
  frame.reportSuggestionsContent = reportSuggestionsContent
  ...
  local issuesScroll = CreateFrame("ScrollFrame", "AnalyzerDPSIssuesScroll", issuesFrame, "UIPanelScrollFrameTemplate")
  issuesScroll:SetPoint("TOPLEFT", 0, -18)
  issuesScroll:SetPoint("BOTTOMRIGHT", -26, 0)

  local issuesContent = CreateFrame("Frame", nil, issuesScroll)
  issuesContent:SetSize(1, 1)
  issuesScroll:SetScrollChild(issuesContent)

  local issuesText = issuesContent:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
  issuesText:SetPoint("TOPLEFT", 0, 0)
  issuesText:SetWidth(issuesFrame:GetWidth() - 40)
  issuesText:SetJustifyH("LEFT")
  issuesText:SetWordWrap(true)
  issuesText:SetText("")

  -- Suggestions tab content - z ikonkami i lepszym formatowaniem
  local suggestionsFrame = CreateFrame("Frame", nil, reportSuggestionsContent)
  suggestionsFrame:SetSize(720, 440)
  suggestionsFrame:SetPoint("TOPLEFT", 16, -96)

  suggestionsFrame.title = suggestionsFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  suggestionsFrame.title:SetPoint("TOPLEFT", 0, 0)
  suggestionsFrame.title:SetText("Sugestie poprawy DPS")
  suggestionsFrame.title:SetTextColor(1.00, 0.85, 0.10)

  local suggestionsScroll = CreateFrame("ScrollFrame", "AnalyzerDPSSuggestionsScroll", suggestionsFrame, "UIPanelScrollFrameTemplate")
  suggestionsScroll:SetPoint("TOPLEFT", 0, -24)
  suggestionsScroll:SetPoint("BOTTOMRIGHT", -26, 0)

  local suggestionsContent = CreateFrame("Frame", nil, suggestionsScroll)
  suggestionsContent:SetSize(suggestionsFrame:GetWidth() - 28, 1)
  suggestionsScroll:SetScrollChild(suggestionsContent)
  
  -- Kontener na sugestie (będzie wypełniony dynamicznie)
  suggestionsFrame.itemsContainer = suggestionsContent
  ...
  Analyzer.ui.suggestionsContainer = suggestionsFrame.itemsContainer
  ...
  local function CreateSoundRow(parent, yOffset, label, key)
    ...
    row.presetBtn = CreateFrame("Button", nil, row, "GameMenuButtonTemplate")
    row.presetBtn:SetSize(90, 20)
    row.presetBtn:SetPoint("LEFT", row.input, "RIGHT", 6, 0)
    row.presetBtn:SetText("Browse...")
    row.presetBtn:SetScript("OnEnter", function(self)
      GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
      GameTooltip:SetText("Click to browse available sounds", 1, 1, 1)
      GameTooltip:AddLine("Choose from 22 pre-configured sound options", 0.8, 0.8, 0.8, true)
      GameTooltip:Show()
    end)
    row.presetBtn:SetScript("OnLeave", function(self)
      GameTooltip:Hide()
    end)
    row.presetBtn:SetScript("OnClick", function(self)
      local menu = {}
      local sounds = Analyzer.RecommendedSounds or {}
      for _, snd in ipairs(sounds) do
        local soundPath = snd.path
        local soundName = snd.name
        table.insert(menu, {
          text = soundName .. " - " .. snd.description,
          notCheckable = true,
          func = function()
            row.input:SetText(soundPath)
            row.input:SetCursorPosition(0)
            local entry = Analyzer:EnsureSoundEntry(key)
            entry.sound = soundPath
          end,
        })
      end
      if #menu > 0 then
        local dropdown = CreateFrame("Frame", "AnalyzerDPSSoundMenu", UIParent, "UIDropDownMenuTemplate")
    ...
    row.test:SetPoint("LEFT", row.presetBtn, "RIGHT", 6, 0)
  ...
  versionText:SetText("Version 0.74")
...
local function CreateMiniLiveWindow()
  ...
  Analyzer:ApplyUISkin(miniWindow, { alpha = 0.95, noBorder = true, noTopLine = true })
  ...
  miniWindow.adviceContainer:SetSize(320, 80)
  miniWindow.adviceContainer:SetPoint("TOP", miniWindow.dpsValue, "BOTTOM", 0, -10)
  Analyzer:ApplyUISkin(miniWindow.adviceContainer, { alpha = 0.5, noBorder = true, noTopLine = true })

  -- Advice Icon (mniejszy)
  miniWindow.adviceIcon = miniWindow.adviceContainer:CreateTexture(nil, "ARTWORK")
  miniWindow.adviceIcon:SetSize(32, 32)
  miniWindow.adviceIcon:SetPoint("LEFT", miniWindow.adviceContainer, "LEFT", 4, 0)
  miniWindow.adviceIcon:Hide()

  -- Advice Text (mniejsza czcionka, większy kontener)
  miniWindow.adviceText = miniWindow.adviceContainer:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  miniWindow.adviceText:SetPoint("LEFT", miniWindow.adviceIcon, "RIGHT", 6, 0)
  miniWindow.adviceText:SetPoint("RIGHT", miniWindow.adviceContainer, "RIGHT", -6, 0)
  miniWindow.adviceText:SetHeight(72)
  miniWindow.adviceText:SetJustifyH("LEFT")
  miniWindow.adviceText:SetJustifyV("TOP")
  ...
  miniWindow.adviceText:SetFont(miniWindow.adviceText:GetFont(), 11, "OUTLINE")
...
function Analyzer:UpdateMiniLiveWindow()
  ...
  -- Pobierz nowe wskazówki z modułu
  local newAdvices = {}
  if self.activeModule and self.activeModule.GetLiveAdviceList then
    newAdvices = self.activeModule.GetLiveAdviceList(self, self.fight) or {}
  elseif self.activeModule and self.activeModule.GetLiveAdvice then
    local singleAdvice = self.activeModule.GetLiveAdvice(self, self.fight)
    if singleAdvice and singleAdvice ~= "" then
      table.insert(newAdvices, { text = singleAdvice, priority = 1 })
    end
  end
  
  -- Aktualizuj aktywne wskazówki - dodaj nowe, usuń wykonane
  self.activeAdvices = self.activeAdvices or {}
  for _, newAdv in ipairs(newAdvices) do
    local exists = false
    for _, active in ipairs(self.activeAdvices) do
      if active.text == newAdv.text then
        exists = true
        active.lastSeen = now
        break
      end
    ...
    if not exists then
      newAdv.addedAt = now
      newAdv.lastSeen = now
      table.insert(self.activeAdvices, newAdv)
    end
  end
  
  -- Usuń wskazówki które nie są już aktualne (nie widziane przez 5s)
  local filtered = {}
  for _, active in ipairs(self.activeAdvices) do
    if (now - active.lastSeen) < 5 then
      table.insert(filtered, active)
    end
  end
  self.activeAdvices = filtered
  
  -- Sortuj po priorytecie (wyższy = ważniejszy)
  table.sort(self.activeAdvices, function(a, b)
    return (a.priority or 1) > (b.priority or 1)
  end)
  
  -- Pokaż do 3 najważniejszych
  local adviceList = {}
  for i = 1, math.min(3, #self.activeAdvices) do
    table.insert(adviceList, self.activeAdvices[i])
  end

  if #adviceList > 0 then
    miniWindow.adviceContainer:Show()
    
    -- Pokaż do 3 wskazówek, oddzielone separatorem
    local displayText = ""
    local maxAdvice = math.min(3, #adviceList)
    for i = 1, maxAdvice do
      if i > 1 then
        displayText = displayText .. "\n"
      end
      displayText = displayText .. adviceList[i].text
    end
    
    miniWindow.adviceText:SetText(displayText)
    
    -- Wyczyść poprzednią ikonę
    miniWindow.adviceIcon:Hide()
    miniWindow.adviceText:ClearAllPoints()
    
    -- Użyj ikony z pierwszej wskazówki (najwyższy priorytet)
    local iconShown = false
    if adviceList[1].spellId then
      local spellTexture = GetSpellTexture(adviceList[1].spellId)
      if spellTexture then
        miniWindow.adviceIcon:SetTexture(spellTexture)
        miniWindow.adviceIcon:Show()
        iconShown = true
      end
    elseif self.activeModule and self.activeModule.GetAdviceSpellIcon then
...
frame:SetScript("OnEvent", function(_, event, ...)
  if event == "ADDON_LOADED" then
    ...
    -- Start mini window update ticker (DPS/advice co 0.5s, score co 5s)
    Analyzer.lastScoreUpdate = 0
    C_Timer.NewTicker(0.5, function()
      Analyzer:UpdateMiniLiveWindow()
      -- Auto-stop walki po 10 sekundach bez castów (minimum 5 sekund walki)
      if Analyzer.fight and Analyzer.fight.lastCastTime then
        local fightDuration = GetTime() - Analyzer.fight.startTime
        local timeSinceLastCast = GetTime() - Analyzer.fight.lastCastTime
        if fightDuration >= 5 and timeSinceLastCast >= 10 then
          Analyzer:EndFight(true) -- true = autoStopped
        end
      end
    end)
  ...
  if event == "COMBAT_LOG_EVENT_UNFILTERED" then
    ...
    if not Analyzer.fight then
      -- Precast detection - SPELL_CAST_START dla channeled (Evocation)
      if (subevent == "SPELL_CAST_START" or subevent == "SPELL_CAST_SUCCESS")
        and isPlayerSource
        and spellId then
        local module = Analyzer.activeModule
        local isPrecombatCast = false
        
        -- Sprawdź czy to Evocation lub inny precast spell
        if module and module.IsPrecombatCastSpell then
          isPrecombatCast = module.IsPrecombatCastSpell(spellId)
        end
        
        if isPrecombatCast or IsHostileFlags(destFlags) then
