...
local SPELL_MIND_BLAST = 8205  -- Base spell ID
local SPELL_MIND_BLAST_RANK_2 = 8206
local SPELL_MIND_BLAST_RANK_3 = 10945
local SPELL_MIND_BLAST_RANK_4 = 10946
local SPELL_MIND_BLAST_RANK_5 = 10947
local SPELL_MIND_BLAST_RANK_6 = 25372
local SPELL_MIND_BLAST_RANK_7 = 25375
local SPELL_MIND_BLAST_RANK_8 = 48126
local SPELL_MIND_BLAST_RANK_9 = 48127
...
local TRACKED_CASTS = {
  ...
  [SPELL_MIND_BLAST_RANK_2] = true,
  [SPELL_MIND_BLAST_RANK_3] = true,
  [SPELL_MIND_BLAST_RANK_4] = true,
  [SPELL_MIND_BLAST_RANK_5] = true,
  [SPELL_MIND_BLAST_RANK_6] = true,
  [SPELL_MIND_BLAST_RANK_7] = true,
  [SPELL_MIND_BLAST_RANK_8] = true,
  [SPELL_MIND_BLAST_RANK_9] = true,
...
function module.TrackSpellCast(analyzer, spellId, timestamp)
  ...
  -- Normalize Mind Blast ranks to base spell ID
  local normalizedSpellId = spellId
  if spellId == SPELL_MIND_BLAST_RANK_2 or spellId == SPELL_MIND_BLAST_RANK_3 or 
     spellId == SPELL_MIND_BLAST_RANK_4 or spellId == SPELL_MIND_BLAST_RANK_5 or
     spellId == SPELL_MIND_BLAST_RANK_6 or spellId == SPELL_MIND_BLAST_RANK_7 or
     spellId == SPELL_MIND_BLAST_RANK_8 or spellId == SPELL_MIND_BLAST_RANK_9 then
    normalizedSpellId = SPELL_MIND_BLAST
  end
  
  fight.spells[normalizedSpellId] = (fight.spells[normalizedSpellId] or 0) + 1
  table.insert(fight.castLog, { spellId = normalizedSpellId, timestamp = now })
  ...
  elseif normalizedSpellId == SPELL_MIND_BLAST then
    ...
    analyzer:AddEventLog(now, "Mind Blast", SPELL_MIND_BLAST)
...
function module.TrackAura(analyzer, subevent, spellId, amount, timestamp)
  ...
  if subevent == "SPELL_AURA_APPLIED"
    ...
    elseif spellId == SPELL_SHADOWFORM then
      analyzer:AddEventLog(now, "Shadowform aktywny", spellId)
...
-- =============================================================================
-- APL (Action Priority List) - Shadow Priest rotation
-- =============================================================================

local APL_PRIORITY = {
  -- Priorytet 1: Shadowfiend
  {
    id = "shadowfiend",
    spellId = SPELL_SHADOWFIEND,
    name = "Shadowfiend",
    condition = function(state)
      return state.shadowfiendReady and state.duration > 10
    end,
    priority = 1,
    category = "cooldown",
    description = "Shadowfiend na cooldown",
  },
  
  -- Priorytet 2: Shadow Word: Pain (brak)
  {
    id = "swp_missing",
    spellId = SPELL_SHADOW_WORD_PAIN,
    name = "Shadow Word: Pain",
    condition = function(state)
      return state.swpMissing and state.duration > 2
    end,
    priority = 2,
    category = "dot",
    description = "Naloż SW:P",
  },
  
  -- Priorytet 3: Vampiric Touch (brak)
  {
    id = "vt_missing",
    spellId = SPELL_VAMPIRIC_TOUCH,
    name = "Vampiric Touch",
    condition = function(state)
      return state.vtMissing and state.duration > 3
    end,
    priority = 3,
    category = "dot",
    description = "Naloż VT",
  },
  
  -- Priorytet 4: Mind Spike z Surge of Darkness
  {
    id = "mind_spike_surge",
    spellId = SPELL_MIND_SPIKE,
    name = "Mind Spike (Surge)",
    condition = function(state)
      return state.surgeOfDarknessActive
    end,
    priority = 4,
    category = "proc",
    description = "Mind Spike instant z Surge of Darkness",
  },
  
  -- Priorytet 5: Devouring Plague (3 Shadow Orbs)
  {
    id = "devouring_plague",
    spellId = SPELL_DEVOURING_PLAGUE,
    name = "Devouring Plague",
    condition = function(state)
      return state.shadowOrbs >= 3 and state.duration > 5
    end,
    priority = 5,
    category = "damage",
    description = "Devouring Plague z 3 orbami",
  },
  
  -- Priorytet 6: Mind Blast
  {
    id = "mind_blast",
    spellId = SPELL_MIND_BLAST,
    name = "Mind Blast",
    condition = function(state)
      return state.mindBlastReady
    end,
    priority = 6,
    category = "damage",
    description = "Mind Blast na cooldown",
  },
  
  -- Priorytet 7: Shadow Word: Death (execute)
  {
    id = "swd",
    spellId = SPELL_SHADOW_WORD_DEATH,
    name = "Shadow Word: Death",
    condition = function(state)
      return state.swdReady and state.duration > 5 -- Simplified: w prawdziwej rotacji <20% HP
    end,
    priority = 7,
    category = "damage",
    description = "SW:D na cooldown",
  },
  
  -- Priorytet 8: Shadow Word: Pain (refresh)
  {
    id = "swp_refresh",
    spellId = SPELL_SHADOW_WORD_PAIN,
    name = "Shadow Word: Pain (refresh)",
    condition = function(state)
      return state.swpExpiring
    end,
    priority = 8,
    category = "dot",
    description = "Odśwież SW:P",
  },
  
  -- Priorytet 9: Vampiric Touch (refresh)
  {
    id = "vt_refresh",
    spellId = SPELL_VAMPIRIC_TOUCH,
    name = "Vampiric Touch (refresh)",
    condition = function(state)
      return state.vtExpiring
    end,
    priority = 9,
    category = "dot",
    description = "Odśwież VT",
  },
  
  -- Priorytet 10: Mind Flay (filler)
  {
    id = "mind_flay",
    spellId = SPELL_MIND_FLAY,
    name = "Mind Flay",
    condition = function(state)
      return true
    end,
    priority = 10,
    category = "filler",
    description = "Mind Flay - główny filler",
  },
}
...
local function GetCurrentState(analyzer, fight)
  local state = {
    duration = 0,
    surgeOfDarknessActive = false,
    swpMissing = false,
    swpExpiring = false,
    vtMissing = false,
    vtExpiring = false,
    shadowOrbs = 0,
    shadowfiendReady = false,
    mindBlastReady = false,
    swdReady = false,
  }
  ...
  -- Sprawdź buffy
  state.surgeOfDarknessActive = CheckPlayerBuff(SPELL_SURGE_OF_DARKNESS)
  
  -- Sprawdź Shadow Orbs
  if CheckPlayerBuff(SPELL_SHADOW_ORB_3) then
    state.shadowOrbs = 3
  elseif CheckPlayerBuff(SPELL_SHADOW_ORB_2) then
    state.shadowOrbs = 2
  elseif CheckPlayerBuff(SPELL_SHADOW_ORB_1) then
    state.shadowOrbs = 1
  end
  
  -- Sprawdź cooldowny
  state.shadowfiendReady = utils.IsSpellReady(SPELL_SHADOWFIEND)
  state.mindBlastReady = utils.IsSpellReady(SPELL_MIND_BLAST)
  state.swdReady = utils.IsSpellReady(SPELL_SHADOW_WORD_DEATH)
  
  -- Sprawdź doty na target
  if UnitExists("target") and state.duration > 2 then
    local hasSWP = CheckTargetDebuff(SPELL_SHADOW_WORD_PAIN)
    local hasVT = CheckTargetDebuff(SPELL_VAMPIRIC_TOUCH)
    
    if not hasSWP then
      state.swpMissing = true
    end
    if not hasVT then
      state.vtMissing = true
    end
    -- TODO: sprawdź czy wygasają (potrzebujemy expirationTime)
  end
...
function module.GetNextAPLAction(analyzer, fight)
  ...
  return APL_PRIORITY[#APL_PRIORITY] -- Mind Flay jako fallback
end
